...
describe('#mutations', () => {
  describe('#SET_AGENTS', () => {
    it('set agent records', () => {
      ...
      mutations[types.default.SET_AGENTS](state, [
        { id: 1, name: 'Agent1', email: 'agent1@chatwoot.vn' },
      ]);
      expect(state.records).toEqual([
        {
          id: 1,
          name: 'Agent1',
          email: 'agent1@chatwoot.vn',
        },
      ]);
    });
  });
  ...
  describe('#ADD_AGENT', () => {
    it('push newly created agent data to the store', () => {
      const state = {
        records: [{ id: 1, name: 'Agent1', email: 'agent1@chatwoot.vn' }],
      };
      mutations[types.default.ADD_AGENT](state, {
        id: 2,
        name: 'Agent2',
        email: 'agent2@chatwoot.vn',
      });
      expect(state.records).toEqual([
        { id: 1, name: 'Agent1', email: 'agent1@chatwoot.vn' },
        { id: 2, name: 'Agent2', email: 'agent2@chatwoot.vn' },
      ]);
    });
  });
  ...
  describe('#EDIT_AGENT', () => {
    it('update agent record', () => {
      ...
      expect(state.records).toEqual([
        { id: 1, name: 'Agent2', email: 'agent2@chatwoot.vn' },
      ]);
    });
  });
